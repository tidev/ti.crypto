...
@Kroll.proxy(creatableInModule = CryptoModule.class,
             propertyAccessors = {"resizeBuffer", "operation", "algorithm",
                                  "options", "key", "initializationVector"})
public class CryptorProxy extends KrollProxy {
  ...
  private class CryptOptions {
    ...
    public byte[] getBytesIn() { return dataIn.getBuffer(); }
    ...
    public boolean isEncrypt() { return operation == CryptoModule.ENCRYPT; }
  }

  private CryptOptions prepareCryptOptions(BufferProxy dataIn, int dataInLength,
                                           BufferProxy dataOut,
                                           int dataOutLength) {
    ...
    o.key = dict.containsKey("key") ? ((BufferProxy)dict.get("key")).getBuffer()
                                    : null;
    o.iv = dict.containsKey("initializationVector")
               ? ((BufferProxy)dict.get("initializationVector")).getBuffer()
               : null;

    o.dataIn = dataIn;
    o.dataInLength =
        dataInLength > 0 || dataIn == null ? dataInLength : dataIn.getLength();
    o.dataOut = dataOut != null ? dataOut : dataIn;
    o.dataOutLength = dataOutLength > 0 || o.dataOut == null
                          ? dataOutLength
                          : o.dataOut.getLength();
  ...
  private PaddedBufferedBlockCipher createCipher(CryptOptions o) {
    ...
    if (o.iv != null) {
      byte[] iv = o.iv;
      // Ensure initializationVector is the same as the block size; 0 fill, if
      // necessary.
  ...
  private int crypt(CryptOptions o) throws Exception {
    ...
    int numBytesWritten =
        cipher.processBytes(encData, 0, dataInLength, buffer, 0);
  ...
  @Kroll.method
  public int encrypt(BufferProxy dataIn,
                     @Kroll.argument(optional = true) int dataInLength,
                     @Kroll.argument(optional = true) BufferProxy dataOut,
                     @Kroll.argument(optional = true) int dataOutLength) {
    try {
      CryptOptions o =
          prepareCryptOptions(dataIn, dataInLength, dataOut, dataOutLength);
  ...
  @Kroll.method
  public int decrypt(BufferProxy dataIn,
                     @Kroll.argument(optional = true) int dataInLength,
                     @Kroll.argument(optional = true) BufferProxy dataOut,
                     @Kroll.argument(optional = true) int dataOutLength) {
    try {
  ...
  @Kroll.method
  public int update(BufferProxy dataIn,
                    @Kroll.argument(optional = true) int dataInLength,
                    @Kroll.argument(optional = true) BufferProxy dataOut,
                    @Kroll.argument(optional = true) int dataOutLength) {
    try {
      ...
      int numBytesWritten =
          cipher.processBytes(encData, 0, o.getDataInLength(), buffer, 0);
      // numBytesWritten += cipher.doFinal(buffer, numBytesWritten);
  ...
  @Kroll.method
  public int finish(BufferProxy dataOut,
                    @Kroll.argument(optional = true) int dataOutLength) {
    try {
      // Get our cipher.
  ...
  @Kroll.method
  public int
  reset(@Kroll.argument(optional = true) BufferProxy initializationBuffer) {
